...
% each control point...
function [sep,stag] = ThwaitesMethod2(xb,yb,velfield,AoA_rad)

%% Define variables
panelLength = sqrt((xb(2:end)-xb(1:end-1)).^2+(yb(2:end)-yb(1:end-1)).^2);

indices = 1:1:length(panelLength);
velfield = 100*velfield;
%x = cumsum(panelLength); % computes total arc-distances

x_S = panelLength/2 + cumsum(panelLength)-panelLength(1);
x_S(1) = 0;
...
% u_6 = u_6(1:end-1);
% u_5 = u_5(1:end-1);
du = gradient(velfield);
i_u5 = cumtrapz(x_S,u_5);
dUedx = du./dx;
...
if velfield(1) < 0
    ...
    stag1 = diff_x_1_intend./slope+x_S(ind_stag_1);
else
    ind_stag = find(velfield<0,1);
    ind_stag_1 = ind_stag-1;
    dUdx = diff(velfield)./dx(1,end-1);
